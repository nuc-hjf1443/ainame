...
async def test_expert_order_report_and_review_flow(session):
    ...
    unused_package = await repo.create_package({
        "name": "未使用精批", "expert_type": "BRAND_CONSULTANT", "price": 299,
        "delivery_days": 5, "description": "未使用服务", "status": "ACTIVE",
    })
    assert await repo.delete_package(unused_package.id) is True
    assert await repo.delete_package(package.id) is False
    assert await repo.delete_package(999999) is None
