...
本文依据 `models/`、`routers/`、`core/`、Alembic 迁移和依赖配置整理，未连接本地 MySQL 实例核验真实数据、表结构和当前迁移版本。数据库实际状态应以目标环境执行 Alembic 检查和数据库查询的结果为准。
...
| 位置 | 当前职责 |
| --- | --- |
| `main.py` | FastAPI 应用入口、生命周期、路由注册、CORS 和静态资源挂载 |
| `routers/` | HTTP 接口、参数编排、认证依赖，以及部分业务状态处理 |
| `schemas/` | Pydantic 请求和响应模型 |
| `models/` | SQLAlchemy ORM 模型、异步引擎和 Session 工厂 |
| `repository/` | 用户、会员、资产、视觉、社区、Marketplace 和管理端数据访问 |
| `core/` | LangGraph 命名、RAG、认证、配额、视觉生成和 Marketplace AI 能力 |
| `alembictable/` | Alembic 异步迁移环境和版本脚本 |
| `rag_worker.py` | RabbitMQ RAG 消费进程 |
| `tests/` | 会员、视觉、Marketplace、社区和静态资源相关测试 |

当前项目已出现 `quota_service`、`marketplace_service`、`brand_kit_service` 等服务模块，但尚未形成完整的 `services/` 分层。部分 Router 仍直接负责配额、订单和状态流转。
...
| 业务分层 | Router、Core、Repository，存在部分 Service | 完整 Router → Service → Repository 分层 |
