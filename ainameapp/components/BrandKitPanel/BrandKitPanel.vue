<template>
  <view class="brand-kit-panel">
    <view class="panel-toolbar">
      <button class="back-btn" @click="$emit('back')">返回重新选择</button>
    </view>
    ...
    <view v-if="!kit" class="panel form-panel">
      ...
      <picker :range="styleOptions" @change="event => form.design_style = styleOptions[event.detail.value]">
        <view class="field picker">{{ form.design_style }}</view>
      </picker>
      <view class="field-label">品牌主色</view>
      <picker :range="colorOptions" @change="event => form.primary_color = colorOptions[event.detail.value]">
        <view class="field picker"><text class="color-dot" :style="{ background: colorValue(form.primary_color) }"></text>{{ form.primary_color }}</view>
      </picker>
      <view class="field-label">生图模型</view>
      <picker :range="modelOptionLabels" @change="event => form.image_model = modelOptions[event.detail.value].value">
        <view class="field picker">{{ modelText(form.image_model) }}</view>
      </picker>
      <button class="primary-btn" :loading="creating" @click="createKit">生成品牌视觉</button>
    </view>
    ...
    <template v-else>
      ...
      <view class="panel status-panel">
        <view class="status-head">
          <view>
            <view class="panel-title">品牌方案</view>
            <view class="status-copy">{{ statusText }}</view>
          </view>
          <button size="mini" class="refresh-btn" :loading="refreshing" @click="refreshKit">刷新状态</button>
        </view>
      ...
      <view class="panel material-panel">
        <view class="material-head">
          ...
          <view class="filters">
            <text :class="filter === 'ALL' ? 'active' : ''" @click="filter = 'ALL'">全部</text>
            <text :class="filter === 'LOGO' ? 'active' : ''" @click="filter = 'LOGO'">Logo</text>
            <text :class="filter === 'BUSINESS_CARD' ? 'active' : ''" @click="filter = 'BUSINESS_CARD'">名片</text>
          </view>
        </view>
...
const createKit = async () => {
  ...
  try {
    kit.value = await http.createBrandKit({
      ...
      naming_asset_id: props.draft.naming_asset_id || null,
      name: props.draft.name,
      moral: props.draft.moral || '',
      category: '企业名',
...
onMounted(async () => {
  form.industry = props.draft.industry_hint || '';
  const active = uni.getStorageSync('activeBrandKit') || {};
  if (active.id && active.name === props.draft.name) {
    try {
